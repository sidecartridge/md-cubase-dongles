...
- **Black dongle (V2)**: an 8-bit state machine that advances on *every* bus cycle,
  ...
  table is in `rp/src/include/cubase2_lut.h`, verified over all 65 536 transitions),
  while a second PIO drives the response on each ROM3 read via the same DMA fetch
  trick.
...
for, and should use this only with software you own.
