...
See also: `programming.md` (full shared-region table and budget rules), `README.md` (high-level region/userfw overview), `AGENTS.md` (overlapping playbook + troubleshooting table), `docs/epics/` (iteration/epic/story/task backlog — run `docs/epics/cockpit.sh` for the live `STATUS.md` dashboard).

## What this app is being built into

This template is being turned into a **Cubase 3 "red dongle" emulator** (Intel 5C060 PLD) for the Atari ST — the MultiDevice answers ROM3 accesses like the original Steinberg copy-protection dongle. The work is tracked in `docs/epics/` (Iterations → Epics → Stories → Tasks). The dongle is a 16-bit state machine compressed to a verified ~24 KB lookup table (`rp/src/include/cubase_lut.h`, generated by `tools/generate_lut.py` and regression-tested by `tools/test_lut.py`); the reverse-engineered sources are not committed (provenance by hash in `tools/reference/PROVENANCE.md`).
...
1. Copies `version.txt` into `rp/` and `target/` (but see the version gotcha below — the m68k build does not read either copy).
...
- The m68k Makefile reads `target/atarist/version.txt`, a separately **checked-in** file (currently stale at `0.0.1dev`) — `build.sh` copies the root `version.txt` only to `target/version.txt`, which nothing reads. The `VERSION_MAJOR/MINOR/PATCH` symbols VASM bakes into the cartridge therefore do not track the root `version.txt`; bump `target/atarist/version.txt` by hand if the ST-side version string matters.
...
- `src/main.s` — m68k cartridge boot + dispatch + terminal. Lives at `$FA0000` in the ST address space (ROM4 cartridge region). Defines the cartridge header (`CA_MAGIC`, `CA_INIT`, …), command magic numbers, and the shared-variable layout used to talk to the RP2040. Commands travel m68k → RP by **reading** addresses inside the ROM3 window (`ROMCMD_START_ADDR = $FB0000`, magic word `$ABCD`) — captured RP-side by `commemul`/`chandler` (below); the RP acknowledges by echoing the request's random token into the shared block.
...
| `$FA2004` | `RANDOM_TOKEN`, `RANDOM_TOKEN_SEED`, reserved slot, 60 × 4 B indexed shared variables | 252 B | fixed-offset metadata block (ends at `$FA20FF`) |
| `$FA2100` | `APP_BUFFERS` (high-res mask table) | 512 B | written by the display code for the 640×400 high-res path; apps that don't use it may reclaim these bytes |
...
- `romemul.c` / `romemul.pio` — the ROM4 read engine: PIO + two chained DMA channels serve every Atari read of the 64 KB window straight from `ROM_IN_RAM` with zero CPU involvement (GPIOs defined in `include/constants.h`). Changes here cause very strange bugs — treat as infrastructure.
- `commemul.c` / `commemul.pio` — the ROM3 command channel: a PIO state machine captures every m68k read in the ROM3 window into a 32 KB DMA ring buffer (ring mode, no IRQs, no CPU on the capture path).
- `chandler.c` — polled command parser/dispatcher on top of `commemul` + `tprotocol`: `chandler_loop()` (called from the app main loop) drains the ring, reassembles and validates commands, fans them out to callbacks registered with `chandler_addCB()`, then acknowledges by writing the random token back to shared memory. **Registering a callback here (as `emul.c` does with `term_command_cb`) is how an app handles its own commands from the Atari.** `chandler.h` is also the canonical home of the shared-region offset constants.
...
Getting an app into the public catalogue that Booster downloads is **one pull request** against [`sidecartridge-microfirmwares-store`](https://github.com/sidecartridge/sidecartridge-microfirmwares-store), adding an origin that points at an `apps.json` you host yourself. The PR appends one entry — `creator.id` (lowercase-kebab, permanent), `creator.name`, `creator.url`, `official: false`, `enabled: true`, and the `url` of your hosted `apps.json` — to `origins/atari-st.json` (or `origins/atari-st-test.json` / `origins/atari-st-dev.json` for the testing/dev channels). Binary and image URLs inside `apps.json` must be HTTPS. Validate before pushing with `python3 tools/build_catalog.py --check --strict` in the store repo; the PR check runs the same validation. The store keeps a pointer, not a copy, so later releases need no further pull request: bump `version.txt`, rebuild, upload the new UF2, update your `apps.json` in place — the catalogue rebuilds daily. Walkthrough: <https://md-store.sidecartridge.com/build/atari-st/11-get-listed.html>; full build track: <https://md-store.sidecartridge.com/build/>
